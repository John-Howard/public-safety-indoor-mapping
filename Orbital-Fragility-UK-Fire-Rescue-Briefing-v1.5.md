...
> **Briefing paper.** Version 1.5, issued 4 August 2026. Horizon-scanning paper, not a formal risk assessment.
...
The clearest dependency is at the call-handling stage. The largest single emergency-service benefit from GNSS comes from public-safety answering points using satellite-derived handset location, in the UK through Advanced Mobile Location, to shorten call and search times (London Economics, 2017; Inside GNSS, 2018). If that degrades, caller location falls back to cell-sector triangulation, which fixes a caller to a district, not a doorway. Call times lengthen and crews are sent out to search.

Control-room mobilising leans on satellite positioning but is not captive to it. National guidance describes systems using an address-based gazetteer and automatic vehicle location to present the nearest resource, with fire control able to override by hand (National Fire Chiefs Council, no date). The gazetteer, GIS risk layers, hydrant data and pre-planned rendezvous points all survive an outage. What is lost is automatic nearest-appliance selection, which reverts to station-area dispatch and human judgement, a change worth roughly 15 to 25 per cent on response times where services have measured it (RadioMobile, 2026).
...
National Fire Chiefs Council (no date) *Use technology to mobilise fire and rescue service resources*. UK Fire and Rescue Service guidance. Available at: <https://www.ukfrs.com/guidance/search/use-technology-mobilise-fire-and-rescue-service-resources> (Accessed: 4 August 2026).
