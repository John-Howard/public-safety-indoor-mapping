...
> **Briefing paper.** Version 1.6, issued 4 August 2026. Horizon-scanning paper, not a formal risk assessment.
...
| **1.6** | 4 August 2026 | Added Lewis (2026) on the critical sizes of satellite constellations. The finding that nine of sixteen constellations individually exceed their critical size, under assumptions favourable to the operators, qualifies the earlier statement that current mitigation practice could hold the trajectory. Step one revised accordingly, with the preprint status and the long-term rather than acute scope of that source stated. |
...
That last point now carries a qualification. Lewis (2026) applies a stability model to sixteen planned or deployed constellations and finds that nine of them individually exceed the critical size at which their own debris population becomes unstable or grows without limit. The finding holds even though each constellation is assessed in isolation, in an orbital environment assumed to contain nothing else, with collision avoidance assumed to work perfectly and disposal compliance set above what regulators require. Real conditions would be less favourable. Size alone does not predict the outcome: one system of 528 satellites breaches its threshold while another of 15,000 does not. The paper also identifies a regulatory gap, in that national regulators assess the debris risk of a single satellite rather than the aggregate risk of a whole constellation, while applications lodged with the United States regulator during 2026 alone cover more than 1.2 million satellites.

Two limits on that source should be stated plainly. It is a preprint and has not yet been peer reviewed, unlike the CRASH Clock paper above. It also concerns the long-term stability of the orbital environment rather than the acute disruption this briefing is mainly about, so it does not change the picture over days and weeks. Its relevance is narrower but still real: it weakens the assumption that current mitigation practice is sufficient to hold the trajectory, and that is the assumption a planner would otherwise rely on.
...
Lewis, H.G. (2026) 'Critical sizes of satellite constellations', *arXiv*:2607.29644 \[astro-ph.EP\]. Preprint, submitted 31 July 2026, not yet peer reviewed. Available at: <https://doi.org/10.48550/arXiv.2607.29644> (Accessed: 4 August 2026).
